...
def load_country_data():
    ...
    for index, row in df.iterrows():
        ...
        if gec.lower() in CountryUtils.gec_to_continent and (iso3 in CountryUtils.restcountries_iso3 or iso3 == 'XKS'):
            ...
            create_country_codes(iso3, gec, row)
            continent = CountryUtils.gec_to_continent[gec.lower()]
            factbook_country_json = proxy.retrieve_factbook_country(gec.lower(), continent)
            ...
            create_country(iso3, factbook_country_json, restcountries_country_json)
